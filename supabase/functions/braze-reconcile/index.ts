...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Braze-API pattern (mirrors braze-worldcup-reconcile):
    // Webhooks are no longer used. A schedule is considered delivered once its
    // send time has passed AND Braze no longer lists it as a pending broadcast
    // (i.e. it fired). It is only cancelled if Braze still has it queued in the
    // past, which means it never fired.
    const stillScheduledIds = new Set<string>(
      ourBroadcasts.map((b: any) => b.schedule_id).filter(Boolean)
    );

    const graceCutoff = new Date(now.getTime() - WEBHOOK_GRACE_MINUTES * 60 * 1000);
    const { data: pastPendingSchedules, error: fetchPastError } = await supabase
      .from('schedule_ledger')
      .select('id, match_id, braze_schedule_id, send_at_utc')
      .lt('send_at_utc', graceCutoff.toISOString())
      .eq('status', 'pending');
    ...
    for (const schedule of pastPendingSchedules || []) {
      const stillQueuedInBraze =
        !!schedule.braze_schedule_id && stillScheduledIds.has(schedule.braze_schedule_id);

      if (!stillQueuedInBraze) {
        // Braze dropped it from the scheduled list after the send time -> it fired
        await supabase
          .from('schedule_ledger')
          .update({ status: 'sent' })
          .eq('id', schedule.id);
        ...
        markedAsSent++;
        console.log(`Match ${schedule.match_id}: send time passed and no longer queued in Braze — marked sent`);
      } else {
        // Still queued in Braze past its send time -> genuinely not delivered
        await supabase
          .from('schedule_ledger')
        ...
        markedAsFailed++;
        console.warn(`⚠️ Match ${schedule.match_id}: still queued in Braze past send time — marking as cancelled`);

        await supabase.from('scheduler_logs').insert({
          function_name: 'braze-reconcile',
          match_id: schedule.match_id,
          action: 'delivery_failed',
          reason: 'Still listed as a pending Braze broadcast past its send time',
          details: {
            braze_schedule_id: schedule.braze_schedule_id,
            send_at_utc: schedule.send_at_utc,
            grace_minutes: WEBHOOK_GRACE_MINUTES,
          },
        });
      }
    }

    console.log(`Processed past schedules: ${markedAsSent} marked sent, ${markedAsFailed} failed`);
