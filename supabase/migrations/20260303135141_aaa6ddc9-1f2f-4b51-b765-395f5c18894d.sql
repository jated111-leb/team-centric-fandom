  ...
  SELECT json_build_object(
    ...
    'schedulerHealth', json_build_object(
      ...
      'stalePendingMatches', (
        ...
        FROM (
          SELECT
            sl.match_id AS "matchId",
            COALESCE(m.home_team, 'N/A') AS "homeTeam",
            COALESCE(m.away_team, 'N/A') AS "awayTeam",
            sl.send_at_utc AS "sendAtUtc",
      ...
      'ledgerDuplicateDetails', (
        ...
        FROM (
          ...
          SELECT
            lc.match_id AS "matchId",
            COALESCE(m.home_team, 'N/A') AS "homeTeam",
            COALESCE(m.away_team, 'N/A') AS "awayTeam",
            lc.schedule_count AS "scheduleCount"
          FROM ledger_counts lc
          LEFT JOIN matches m ON m.id = lc.match_id
          ORDER BY lc.schedule_count DESC
          LIMIT 10
        ) t
      )
    ),
