-- Fix fan trap in analytics: filter by send events only
-- Without this filter, a single notification generates multiple rows
-- (Send, Delivery, Open, etc.) and all counts are inflated.
--
-- Also fixes:
-- - multiMatchUsers: now counts across entire period, not just same-day
-- - avgWebhookLatency: computed from data instead of hardcoded 0
-- - get_match_performance: event type filter + handles denormalized field inconsistencies
  ...
  SELECT json_build_object(
      ...
      SELECT json_build_object(
        ...
        'todayUsers', (
          ...
          WHERE sent_at::date = CURRENT_DATE
            AND braze_event_type ILIKE '%send%'
        ),
        'multiMatchUsers', (
          SELECT COUNT(DISTINCT external_user_id)
          FROM (
    ...
    'schedulerHealth', json_build_object(
      ...
      'scheduleLedgerDuplicates', (
        -- Note: schedule_ledger has UNIQUE(match_id) so this will always be 0
        -- Kept for confirmation that the safeguard is working
        SELECT COUNT(*)
        FROM (
          SELECT match_id
      ...
      'topAnomalies', (
        ...
        FROM (
          ...
          WHERE ns.sent_at BETWEEN v_start_date AND v_end_date
            ...
            AND ns.braze_event_type ILIKE '%send%'
...
-- Fix get_match_performance: filter by send events, handle denormalized fields
...
BEGIN
  ...
  FROM (
    SELECT
      match_id as "matchId",
      COALESCE(MODE() WITHIN GROUP (ORDER BY home_team), 'N/A') as "homeTeam",
      COALESCE(MODE() WITHIN GROUP (ORDER BY away_team), 'N/A') as "awayTeam",
      COALESCE(MODE() WITHIN GROUP (ORDER BY competition), 'N/A') as "competition",
